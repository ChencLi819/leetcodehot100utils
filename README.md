...
- **内置编译器**：答题区代码可直接编译运行（Java 调用 JDK 内置 JavaCompiler，Python 调用本机解释器），支持力扣风格代码——自动注入常用 import 与 TreeNode/ListNode/Node 等数据结构
- **训练骨架模板**：每题模板自动生成本题的类与方法签名骨架（方法体为 TODO），像力扣一样刷题
- **IDEA 式编辑体验**：载入题解自动合并（保留 main 函数）并补全 import；`sout`/`psvm`/`fori`/`100.sout` 等快速输入；
